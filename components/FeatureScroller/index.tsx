...
const FeatureScroller: React.FC<GuaranteeScrollerProps>  = ({ title = "Order guarantee", items = [] }) => {
  ...
  return (
    <div className=" p-4 rounded-md w-full max-w-xl mt-4">
      <div className="flex items-center gap-2 text-green-600 font-semibold text-sm mb-2">
        <div className="flex text-[22px] font-medium">{title} <Image
      ...
      <div className="relative flex items-center">
        ...
        >
          {items.map((item, idx) => (
            <span
              key={idx}
              className="bg-green-600 text-white text-sm px-4 py-2 rounded-md whitespace-nowrap"
            >
              {item}
            </span>
          ))}
